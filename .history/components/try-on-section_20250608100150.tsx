            ...
            <CardContent>
              {result ? (
                <div className="space-y-4">
                  {/* 优先检查 imageUrl，然后是 message 中的 URL */}
                  {result.imageUrl && result.imageUrl.startsWith('http') ? (
                    <div className="space-y-4">
                  ...
                  ) : result.message && result.message.startsWith('http') ? (
                    <div className="space-y-4">
                      <div className="relative">
                        <img
                          src={result.message}
                          ...
                          className="w-full h-auto rounded-lg border border-border"
                          onError={(e) => {
                            console.log('图片加载失败:', result.message);
                            const target = e.target as HTMLImageElement;
                            const parent = target.parentElement;
                            if (parent) {
                              parent.innerHTML = `
                                <div class="bg-muted p-4 rounded-lg">
                                  <p class="text-sm text-muted-foreground mb-2">图片加载失败，请尝试直接访问链接：</p>
                                  <a href="${result.message}" target="_blank" class="text-primary hover:underline text-sm break-all">
                                    ${result.message}
                                  </a>
                                  <p class="text-xs text-muted-foreground mt-2">提示：可能是网络问题或图片链接暂时失效</p>
                                </div>
                          ...
                          onLoad={() => {
                            console.log('图片加载成功:', result.message);
                          }}
                        />
                      </div>
                      <div className="flex gap-2">
                        ...
                        <Button
                          ...
                          size="sm"
                          onClick={async () => {
                            if (result.message && navigator.share) {
                              try {
                                await navigator.share({
                                  title: 'StyleMix AI - 我的试衣效果',
                                  text: '看看我用AI生成的试衣效果！',
